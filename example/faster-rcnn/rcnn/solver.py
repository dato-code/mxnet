...
class Solver(object):
    ...
    def get_params(self, grad_req):
        ...
        arg_shapes, out_shapes, aux_shapes = self.symbol.infer_shape(data=(1, 3, 224, 224),
                                                                     gt_boxes=(1, 5),
                                                                     im_info=(1, 3),
                                                                     gt_pad=(1,))
        if grad_req != 'null':
            ...
            for name, shape in zip(arg_names, arg_shapes):
                if not (name.endswith('data') or name.endswith('im_info') or
                        name.endswith('label') or name.endswith('target') or
                        name.startswith('gt_') or
    ...
    def fit(self, train_data,
        ...
        self.optimizer = mx.optimizer.create(self.optimizer, rescale_grad=1.0, sym=self.symbol, **self.kwargs)
            ...
            for databatch in train_data:
                ...
                for key, arr in update_dict.items():
                    if key.startswith("rpn"):
                        print "%s grad: %.5f" % (key, mx.nd.norm(arr).asscalar() / np.prod(arr.shape))
                    self.updater(key, arr, self.arg_params[key])

                label = output_dict['roi_data_labels']
                pred = output_dict['cls_prob_output']
                bb_target = output_dict['roi_data_bbox_targets']
