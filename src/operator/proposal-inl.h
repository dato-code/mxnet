...
struct ProposalParam : public dmlc::Parameter<ProposalParam> {
  ...
  DMLC_DECLARE_PARAMETER(ProposalParam) {
    DMLC_DECLARE_FIELD(rpn_pre_nms_top_n).set_default(12000)
    .describe("Number of top scoring boxes to keep after applying NMS to RPN proposals");
    DMLC_DECLARE_FIELD(rpn_post_nms_top_n).set_default(2000)
    .describe("Overlap threshold used for non-maximum suppresion(suppress boxes with IoU >= this threshold");
    ...
    .desctibe("The size of the receptive field each unit in the convolution layer of the rpn, for example the product of all stride's prior to this layer.");
  }
};
...
class ProposalOp : public NativeOpBase<xpu> {
 public:
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    scores_shape = Shape4(scores_shape[0], scores_shape[1]/2, scores_shape[2]. scores_shape[3]);
    real_t* foreground_score_pointer = in_data_ptr[0] + (scores_shape[0] * scores_shape[1] * scores_shape[2] * scores_shape[3]); 

    Tensor<xpu, 4> scores =  Tensor<xpu, 4>(foreground_score_pointer, scores_shape);
    Tensor<xpu, 4> bbox_deltas = Tensor<xpu, 4>(in_data_ptr[1], in_data[1].shape_);
    Tensor<xpu, 2> im_info = Tensor<xpu, 2>(in_data_ptr[2], in_data[2].shape_);

    Tensor<xpu, 3> out = Tensor<xpu, 3>(out_data_ptr[0], out_data[0].shape_);
    Tensor<xpu, 2> workspace_proposals = Tensor<xpu, 2>(out_data_ptr[1], out_data[1].shape_);
    Tensor<xpu, 2> workspace_nms = Tensor<xpu, 2>(out_data_ptr[2],out_data[2].shape_ ); 

    size_t height = scores.size(2);
    size_t width = scores.size(3);

    
    generate_anchors(param_.base_anchor, param_.ratios, param._scales, &workspace_proposals);
  ...
  wide_bbox_transform_inv(workspace_proposals, deltas, &(workspace_proposals));
  clip_boxes(Shape2(im_info[0][0],im_info[0][1]), &(workspace_proposals));

  float scale = im_info[0][2];
  float out_size;
  Tensor<xpu, 1> output = workspace_nms[4];

  NonMaximumSuppression( workspace_proposals, param._threshold, param_.nms_min_size * scale, param_.pre_nms_top_n, param_.post_nms_top_n, 
                        &workspace_nms, &output, &out_size);
...
class ProposalProp : public OperatorProperty {
 public:
  ...
  bool InferShape(std::vector<TShape> *in_shape,
    ...
    using namespace mshadow;
    CHECK_EQ(in_shape->size(), 3) << "Input:[scores, deltas, im_info]";
    const TShape &dshape = in_shape->at(0);
    if (dshape.ndim() == 0) return false;
    SHAPE_ASSIGN_CHECK(in_shape, 1, Shape4(dshape[0], dshape[1] * 2, dshape[2], dshape[3]));
    SHAPE_ASSIGN_CHECK(in_shape,2, Shape2(1,3));
    out_shape->clear();
    out_shape->push_back(Shape3(1,param_.rpn_post_nms_top_n,5));
    // Shape of input_data[0] is (1,2 * A, H, W), where A is number of anchors 
    // generated from base_anchor, H is number of positions of each anchor in y axis,
    // W is the same in x-axis. 
  ...
  std::vector<std::string> ListOutputs() const override {
      return {"proposals", "workspace_proposals", "workspace_nms" };
    }
  }
