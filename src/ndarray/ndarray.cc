...
void NDArray::SyncCopyFromSFrame(const graphlab::flexible_type *data, size_t size,
                                 size_t idx, size_t batch_size, size_t* field_length_p) const {
  CHECK_GE(size, 1);
  this->WaitToWrite();
  TShape dshape = this->shape();
  TBlob dst = this->data();
  CHECK_EQ(this->ctx().dev_type, Context::kCPU) << "Only support copy SFrame to CPU NDarray";
  CHECK_EQ(dst.type_flag_, mshadow::DataType<float>::kFlag);

  for (size_t i = 0; i < size; ++i) {
    CHECK_NE(data[i].get_type(), graphlab::flex_type_enum::UNDEFINED) << "Missing value is not supported. Please use fillna() or dropna() to remove missing values.";
  }
  ...
  if (size ==  1 && (type == graphlab::flex_type_enum::FLOAT || type == graphlab::flex_type_enum::INTEGER)) {
    auto shape = mshadow::Shape1(dshape[0]);
    mshadow::Tensor<cpu, 1> obj = dst.GetWithShape<cpu, 1, float>(shape);
    obj[idx] = (float)(data[0]);
    return;
  }
  // Case 3: Array type or mixed types
  auto shape = mshadow::Shape2(dshape[0], dshape.Size() / dshape[0]);
  mshadow::Tensor<cpu, 2> obj = dst.GetWithShape<cpu, 2, float>(shape);
  size_t pos = 0;
  for (size_t i = 0; i < size; ++i) {
    auto type = data[i].get_type();
    if (type == graphlab::flex_type_enum::VECTOR) {
      const graphlab::flex_vec& v = data[i].to<graphlab::flex_vec>();
      CHECK_EQ(v.size(), field_length_p[i]);
      for (size_t j = 0; j < v.size(); ++j) {
        obj[idx][pos++] = (float)(v[j]);
      }
    } else if (type == graphlab::flex_type_enum::INTEGER ||
               type == graphlab::flex_type_enum::FLOAT) {
      obj[idx][pos++] = (float)(data[i]);
    } else {
