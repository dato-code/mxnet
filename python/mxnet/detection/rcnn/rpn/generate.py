...
def generate_detections(detector, test_data, rpn_out_path, vis=False):
    """
    ...
    i = 0
    imdb_boxes = list()
    num_images = len(test_data.index)
    for databatch in test_data:
        if i % 10 == 0:
            print 'generating detections {}/{}'.format(i, num_images)
        ...
        imdb_boxes.append(list(boxes))
        if vis:
            dets = np.hstack((boxes * scale, scores))
            vis_detection(databatch.data['data'], dets, thresh=0.9)
        i += 1

    assert len(imdb_boxes) == num_images, 'calculations not complete'
    #rpn_folder = ('rpn_data'
    #if not os.path.exists(rpn_folder):
    #    os.mkdir(rpn_folder)
    out_sf = gl.SFrame({"boxes" : imdb_boxes})
    out_sf.save(rpn_out_path)
    return out_sf
