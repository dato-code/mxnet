...
def get_minibatch(roidb, num_classes, mode='test'):
    ...
    if config[cfg_key].HAS_RPN:
        ...
        if mode == 'train':
            ...
            boxes = np.vstack(roidb[0]['boxes'])
            gt_classes = np.asarray(roidb[0]['gt_classes'])
...
def get_image_array(roidb, scales, scale_indexes):
    ...
    for i in range(num_images):
        ...
        #    im = im[:, ::-1, :]
        im = roidb[i]["image"]
        im = Image.open(_StringIO.StringIO(im._image_data))
...
def sample_rois(roidb, fg_rois_per_image, rois_per_image, num_classes):
    ...
    labels = np.asarray(roidb['max_classes'])
    overlaps = np.asarray(roidb['max_overlaps'])
    rois = np.vstack(roidb['boxes'])
    ...
    tmp_target = np.vstack(roidb['bbox_targets'])
    bbox_targets, bbox_inside_weights = \
        expand_bbox_regression_targets(tmp_target[keep_indexes, :], num_classes)
